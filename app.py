...
<table id="bugTable">
<thead><tr><th>ID</th><th>Title</th><th>description</th><th>Status</th><th>Priority</th><th>Created</th><th>Action</th></tr></thead>
<tbody></tbody></table>
<script>
async function loadBugs(){
    ...
    bugs.forEach(b => {
        ...
        <td>${b.id}</td><td>${b.title}</td><td>${b.description}</td><td>${b.status}</td><td>${b.priority}</td><td>${b.created_at}</td>
        <td>
...
def get_bugs():
    ...
    bugs = [dict(id=r[0],title=r[1],description=r[2],status=r[3],priority=r[4],created_at=r[5]) for r in c.fetchall()]
    conn.close()
    return jsonify(bugs)
...
def add_bug():
    ...
    c.execute("INSERT INTO bugs (title, description, priority, created_at) VALUES (?,?,?,?)",
              (data['title'], data.get('description', ''), data.get('priority', 'Medium'), datetime.now().strftime('%Y-%M-%D %H:%M:%S')))
    conn.commit()
    bug_id = c.lastrowid
    conn.close()
